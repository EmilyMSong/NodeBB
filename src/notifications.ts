...
export async function pushGroup(notification, groupName) {
    if (!notification) {
        return;
    }
...
export async function markReadMultiple(nids, uid) {
    ...
    const relatedNids = await findRelated(mergeIds, `uid:${uid}:notifications:unread`);
